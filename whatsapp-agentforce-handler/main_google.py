...
def handle_voice_message(from_number: str, message_sid: str, media_url: str, content_type: str):
    ...
    try:
        ...
        transcribed_text, detected_language = google_stt(audio_data, content_type)
        ...
        logger.info(f"Generating voice response in background (language: {detected_language})...")
        try:
            audio_base64 = google_tts(response_text, detected_language)
...
def google_stt(audio_data: bytes, content_type: str) -> Tuple[str, str]:
    """Convert speech to text using Google Cloud STT
    
    CRITICAL FIX: Google STT doesn't properly transcribe OGG_OPUS audio from WhatsApp,
    even though it processes it and detects the language. Solution: Convert to LINEAR16/WAV first.
    """
    try:
        ...
        logger.error(f"Request error in Google STT: {str(e)}")
        raise Exception(f"Failed to call Google STT API: {str(e)}")
    except Exception as e:
        logger.error(f"Error in google_stt: {str(e)}", exc_info=True)
        raise
